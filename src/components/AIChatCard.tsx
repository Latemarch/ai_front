...
export default function AIChatCard({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("relative w-full h-full  overflow-hidden ", className)}>
      ...
      {/* Inner Card */}
      <div className="relative flex flex-col w-full h-full border border-white/10 bg-black/30 overflow-hidden backdrop-blur-xl">
        {/* Header */}
        ...
        {/* Messages */}
        <div className="w-full h-full px-2 py-3 overflow-y-auto  text-sm flex flex-col relative ">
          {messages.map((msg, i) => (
            <motion.div
              key={i}
                ...
                <div className="prose prose-sm prose-invert max-w-none">
                  <ReactMarkdown
                    components={{
                      ...
                      pre: ({ children }) => (
                        <pre className="bg-white/10 p-2 rounded mb-2 overflow-x-auto">
                          {children}
                        </pre>
                      ),
                    }}
